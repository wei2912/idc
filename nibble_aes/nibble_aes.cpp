...
diffs differences(const nibs ns0, const nibs ns1) {
    diffs ds;
    for (int i = 0; i < 16; ++i) ds[i] = ns0[i] != ns1[i];
    return ds;
}

nibs nibble_sub(const nibs ns) {
    auto os = ns;
    for (int i = 0; i < 16; ++i) os[i] = SBOX[ns[i]];
    return os;
}

nibs inv_nibble_sub(const nibs ns) {
    auto os = ns;
    for (int i = 0; i < 16; ++i) os[i] = INV_SBOX[ns[i]];
    return os;
}
...
nibs mix_column(const nibs ns) {
    ...
    os[12] = M1[ns[12]] ^ M1[ns[13]] ^ M4[ns[14]] ^ M9[ns[15]];
    os[13] = M1[ns[12]] ^ M4[ns[13]] ^ M9[ns[14]] ^ M1[ns[15]];
    os[14] = M4[ns[12]] ^ M9[ns[13]] ^ M1[ns[14]] ^ M1[ns[15]];
    os[15] = M9[ns[12]] ^ M1[ns[13]] ^ M1[ns[14]] ^ M4[ns[15]];
    return os;
}
...
nibs inv_mix_column(const nibs ns) {
    ...
    os[12] = M8[ns[12]] ^ M12[ns[13]] ^ M7[ns[14]] ^ M7[ns[15]];
    os[13] = M12[ns[12]] ^ M7[ns[13]] ^ M7[ns[14]] ^ M8[ns[15]];
    os[14] = M7[ns[12]] ^ M7[ns[13]] ^ M8[ns[14]] ^ M12[ns[15]];
    os[15] = M7[ns[12]] ^ M8[ns[13]] ^ M12[ns[14]] ^ M7[ns[15]];
    return os;
}

nibs key_addition(const nibs ks, const nibs ns) {
    nibs os;
    for (int i = 0; i < 16; ++i) os[i] = ks[i] ^ ns[i];
    return os;
}
